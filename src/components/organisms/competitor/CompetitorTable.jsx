...
import { useCompetitorStore, COMPETITOR_STATUS_CONFIG } from "../../../store/competitorStore";

const CompetitorTable = ({ filtered, onOpenStatusModal }) => {
  const { loading, dataLoaded } = useCompetitorStore();

  if (loading) {
    return (
      <div className="text-center py-5">
        <Spinner color="primary" style={{ width: "3rem", height: "3rem" }} />
        <p className="mt-3 text-muted">Memuat data kompetitor...</p>
      </div>
    );
  }
  ...
  if (!dataLoaded) {
    return (
      <div className="text-center py-5" style={{ color: "#94a3b8" }}>
        <i className="fas fa-store-alt mb-3" style={{ fontSize: "3rem", opacity: 0.3 }} />
      ...
      <Table hover responsive className="mb-0" style={{ fontSize: "0.875rem" }}>
        <thead style={{ background: "#f8fafc", borderBottom: "2px solid #e2e8f0" }}>
          <tr>
            ...
            <th className="kost-th">Nama Laundry</th>
        ...
        <tbody>
          {filtered.map((item, idx) => {
            const cfg = COMPETITOR_STATUS_CONFIG[item.status] ?? COMPETITOR_STATUS_CONFIG["potential"];
            return (
              <tr key={item.id} style={{ transition: "background 0.15s" }}>
                ...
                <td className="px-3 py-3">
                  ...
                  <div className="text-muted small">{item.category || "Laundry"}</div>
                </td>
                <td className="px-3 py-3">
                  <div
                    style={{
