      ...
      <CardBody className="py-3">
        <div className="d-flex align-items-start justify-content-between">
          <div className="d-flex align-items-start overflow-hidden">
            <div className="service-thumbnail mr-3 rounded-lg overflow-hidden flex-shrink-0 bg-light d-flex align-items-center justify-content-center" style={{ width: 50, height: 50 }}>
              {service.gambar ? (
            ...
            <div className="flex-grow-1 overflow-hidden">
              <div className="d-flex align-items-center mb-1">
                <span className="font-weight-bold text-dark line-clamp-1 mr-2" style={{ fontSize: '0.95rem' }}>{service.nama}</span>
          ...
          <UncontrolledDropdown>
            ...
            <DropdownMenu className="dropdown-menu-arrow" right style={{ borderRadius: '12px', zIndex: 1050 }}>
              <DropdownItem onClick={(e) => { e.stopPropagation(); onEdit(service); }} className="py-2">
                <i className="fas fa-cog text-primary mr-2" /> <span className="small">Konfigurasi</span>
              </DropdownItem>
              <DropdownItem divider />
              <DropdownItem onClick={(e) => { e.stopPropagation(); onDelete(service.id); }} className="py-2">
                <i className="fas fa-trash text-danger mr-2" /> <span className="small">Hapus</span>
              </DropdownItem>
            </DropdownMenu>
          </UncontrolledDropdown>
        </div>
