...
const MessageMedia = ({ messageId, phone, mediaUrl, setPreviewImageUrl, mediaType }) => {
...
const RightPanel = ({ messages, selectedContact, newMessage, setNewMessage, onSendMessage, onDeleteMessage, setShowChat, onLoadMore }) => {
    ...
    useEffect(() => {
        const chatContainer = document.getElementById('chat-messages');
        if (chatContainer) {
             // only autoscroll if user was already at the bottom (or close to it)
          ...
          {pendingFile && (
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '1rem', gap: '10px' }}>
                ...
                >
                  {isSending ? <i className="fas fa-circle-notch fa-spin mr-2"></i> : <i className="fas fa-paper-plane mr-2"></i>}
                  Kirim Sekarang
                </button>
              </div>
            </div>
      ...
      <div className="message-input-area" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
        ...
        {contextMenu && (
          ...
          >
            ...
            >
              <i className="fas fa-reply mr-2 text-primary" style={{ width: '16px' }}></i> Balas
            </div>
            <div 
              style={{ padding: '0.5rem 1rem', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
            ...
            >
              <i className="fas fa-trash-alt mr-2 text-danger" style={{ width: '16px' }}></i> Hapus (Tarik)
            </div>
          </div>
        )}
