...
import { Card, CardHeader, CardBody, Badge, Button, Input } from 'reactstrap';
  ...
  return (
    <Card className="shadow-premium border-0 glass-card h-100" style={{ border: "2px solid #5e72e4" }}>
      ...
      <CardBody className="text-center py-5 px-4">
          ...
          <div className="mb-4 fade-in">
            ...
            {lastScanTime && (
                <div className="text-muted text-xs mt-2 italic font-weight-bold">
                    <i className="far fa-clock me-1" /> Terdeteksi: {moment(lastScanTime).format("HH:mm:ss")}
                </div>
            )}
          </div>
        ) : (
        ...
        >
          <i className="fas fa-plus-circle me-2" /> Daftar Kartu Baru
        </Button>
      </CardBody>
      <style>{`
