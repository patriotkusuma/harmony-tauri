...
### 🤖 AI Blog Generator
Endpoint ini memanfaatkan **Google Gemini AI** untuk membuat artikel blog secara otomatis seputar laundry, dilengkapi *soft-selling* Harmony Laundry dan emoji di dalam kontennya. Blog yang dihasilkan selalu berstatuts **draft** agar admin dapat mereview sebelum dipublish.

- **URL:** `POST /api/v2/blogs/generate-ai`
- **Auth:** Wajib Bearer Token
- **Content-Type:** `multipart/form-data`
- **Form-Data Params:**
  - `user_id` (integer, wajib): ID user yang akan tercatat sebagai penulis artikel.
  - `topic` (string, wajib): Topik yang diinginkan (misalnya: `"baju kasmir"`, `"cuci karpet"`, `"merawat gaun pengantin"`, dsb). Bebas topik apapun selagi masih seputar laundry.
- **Setup:** Pastikan env variable `GEMINI_API_KEY` sudah diisi dengan API Key dari [https://aistudio.google.com/apikey](https://aistudio.google.com/apikey).
- **Response Success (200 OK):** AI akan membuatkan judul, meta_description, dan keseluruhan konten HTML secara otomatis.
  ```json
  {
    "message": "blog berhasil di-generate secara AI sebagai draft",
    "data": {
      "id": 5,
      "user_id": 1,
      "title": "😱 Baju Kasmir Kamu Rusak? Ini Cara Cuci yang Aman (Jangan Salah!)",
      "slug": "baju-kasmir-kamu-rusak-ini-cara-cuci-yang-aman",
      "meta_title": "Cara Mencuci Baju Kasmir yang Aman - Harmony Laundry",
      "meta_desc": "Salah cuci baju kasmir bisa merusaknya selamanya! Simak tips aman dari Harmony Laundry.",
      "thumbnail": null,
      "content": "<h2>🧥 Kasmir: Mewah Tapi Manja!</h2>\n<p>Baju berbahan kasmir memang terasa lembut...",
      "status": "draft",
      "created_at": "2026-03-04T23:00:00Z",
      "updated_at": "2026-03-04T23:00:00Z"
    }
  }
  ```
- **Response Error (500 Internal Server Error):** Jika API Key tidak valid atau quota habis.

#### Cara mendapatkan API Key Gemini (GRATIS):
1. Buka [https://aistudio.google.com/apikey](https://aistudio.google.com/apikey) (login dengan Google Account).
2. Klik **"Create API Key"** → Pilih project Google Cloud kamu (atau buat baru).
3. Salin API Key yang muncul dan paste ke `GEMINI_API_KEY` di file `.env` atau `docker-compose.yml`.

> ⚠️ **Catatan:** Subscription Gemini berbayar (Gemini Advanced / Google AI Studio Pro) memberi *rate limit* yang jauh lebih tinggi dari free tier. Namun untuk penggunaan blog generation saja, **free tier sudah lebih dari cukup**.
