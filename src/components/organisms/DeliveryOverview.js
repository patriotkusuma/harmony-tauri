  ...
  return (
    <Card className="glass-card shadow-premium border-0 mb-4 overflow-hidden">
      ...
      <CardBody className="p-0">
         <Table className="align-items-center table-flush border-0" responsive>
            <thead className="thead-light">
               <tr style={{ background: 'rgba(0,0,0,0.05)' }}>
                  ...
                  <th className="font-weight-bold text-dark py-3 border-0 text-right">Status</th>
               </tr>
            </thead>
            <tbody>
                       ...
                       <tr key={index} className="transition-all hover-translate-up cursor-pointer" style={{ transition: 'all 0.2s' }}>
                          <td className="py-3 border-0">
                             <div className="d-flex align-items-center">
                                <span className={`mr-2 rounded-circle ${isDiambil ? 'bg-success' : 'bg-warning'}`} style={{ width: '8px', height: '8px', flexShrink: 0 }} />
                                <strong className="text-dark" style={{ color: '#000' }}>{ambil.customer.nama}</strong>
                             </div>
                             {ambil.antar === 1 && (
                                <small className="text-info d-block ml-3">
                                   <i className="fas fa-truck-pickup mr-1 mt-1" /> Delivery service
                                </small>
                             )}
                          </td>
                          ...
                          <td className="py-3 border-0 text-right">
