    ...
    return (
        <Modal isOpen={isOpen} toggle={toggle} centered size="lg" contentClassName="border-0 shadow-lg rounded-xl overflow-hidden">
            <ModalHeader toggle={toggle} className="bg-gradient-info text-white border-0 py-4">
                <h4 className="modal-title font-weight-bold ms-2">Detail Transaksi Pembayaran</h4>
            </ModalHeader>
                ...
                {loading ? (
                    ...
                    <div className="p-4">
                        <Row className="mb-4 align-items-center">
                            <Col md="6">
                                <div className="d-flex align-items-center mb-3">
                                    <div className="avatar avatar-md rounded-circle shadow border-2 border-white bg-white overflow-hidden d-flex align-items-center justify-content-center text-primary font-weight-bold me-3" style={{ width: '60px', height: '60px' }}>
                                        <CustomerAvatar customer={detail.customer} />
                                    </div>
                                    <div>
                                        <h4 className="mb-0 font-weight-bold text-dark">{detail.customer?.nama}</h4>
                                        <p className="text-muted mb-0 small"><i className="fab fa-whatsapp me-1 text-success" />{detail.customer?.telpon}</p>
                                    </div>
                                </div>
                            </Col>
                        ...
                        <div className="table-responsive rounded-lg border">
                            <Table className="align-middle table-flush mb-0">
                                <thead className="table-light">
                                    <tr>
                                        <th className="py-3 border-0">ID Pesanan</th>
                                        <th className="py-3 border-0 text-end">Alokasi Bayar</th>
                                        <th className="py-3 border-0 text-end">Sisa Kurang</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {detail.detail_payments?.map((dp, i) => (
                                        <tr key={i}>
                                            <td className="font-weight-bold text-dark py-3">
                                                <i className="ni ni-paper-diploma text-info me-2" />
                                                {dp.pesanan?.kode_pesan || `#${dp.id_pesanan}`}
                                            </td>
                                            <td className="text-end py-3 text-success font-weight-bold">
                                                <RupiahFormater value={dp.total_bayar} />
                                            </td>
                                            <td className="text-end py-3 text-muted font-weight-bold">
